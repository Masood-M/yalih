...
def listandscan(path):
	...
	with open("scanlogs/Yara-report.log", "w") as f:
		print "\n===================================== Yara =====================================" 
		f.write("======================================Yara======================================\n\n")
		f.write(datetime.datetime.now().strftime("%A, %d %B %Y %I:%M:%S%p") + "\n\n")
		f.write("--------------------------------------------------------------------------------\n\n")
		os.system("find . -type f -size 0k -exec rm {} \; | awk '{ print $8 }'")
		process = subprocess.Popen("yara -r yrules/rules.yara " + path[:-1], shell=True, stdout=subprocess.PIPE)
		for line in iter(process.stdout.readline, ''):
			sys.stdout.write(line)
			f.write(line)
		f.write("\nyara -r yrules/rules.yara " + path[:-1] + "\n\n")
